...
public class V1CouponsController {
    ...
    @PostMapping("")
    public List<String> createCoupon(@RequestParam("coupon-size") int size,
                                     @RequestParam("expired-datetime") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime expired) {
        return couponService.createCoupons(size, expired);
    }
